import re
from app.models import transcribe, ipa_vowels, voicing_dict, voi_dict_rev, vocs
...
def transcribe_text_Cracow(text):
    ...
    # phonemic transcription of words
    # taken individually (regional processes not applied)
    ph_words = []
    if words:
        for word in words:
    ...
    for i in range(len(ph_words) - 1):
        if (
            ...
            ph_words[i] = (ph_words[i][:-4]) + \
                          (voi_dict_rev[(ph_words[i][-4:-2])]) + \
                          (voi_dict_rev[(ph_words[i][-2:])])
        elif (
            ...
            ph_words[i] = (ph_words[i][:-3]) + \
                          (voi_dict_rev[(ph_words[i][-3:-1])]) + \
                          (voi_dict_rev[(ph_words[i][-1])])
        elif (
                len(ph_words[i]) >= 3 and
                ph_words[i][-3] in voi_dict_rev and
                ph_words[i][-2:] in voi_dict_rev and
                len(ph_words[i + 1]) > 1 and
                (ph_words[i + 1][0] in voicing_dict or
                 ph_words[i + 1][0] in vocs or
                 ph_words[i + 1][0] in ipa_vowels)
        ):
            ph_words[i] = (ph_words[i][:-3]) + \
                          (voi_dict_rev[(ph_words[i][-3])]) + \
                          (voi_dict_rev[(ph_words[i][-2:])])
        elif (
            ...
            ph_words[i] = (ph_words[i][:-1]) + \
                          (voi_dict_rev[(ph_words[i][-1])])

    # Regressive Final Devoicing before prepositions
    # 'w' and 'z' standing before voiceless consonants
    for i in range(len(ph_words) - 1):
        if (
                    (ph_words[i][-1] in voicing_dict.keys() or
                     ph_words[i][-2:] in voicing_dict.keys()) and
                    (len(ph_words[i + 1]) == 1 and
                     ph_words[i + 1] in voicing_dict.keys()) and
                    ph_words[i + 2] and
    ...
    # Regressive Final Revoicing before prepositions
    # 'w' and 'z' standing before voiced consonants or sonorants
    for i in range(len(ph_words) - 2):
        if (
                ph_words[i][-1] in voi_dict_rev and
                (ph_words[i + 1][0] in voicing_dict
                 and len(ph_words[i + 1]) == 1) and
                (ph_words[i + 2][0] in ipa_vowels or
                 ph_words[i + 2][0] in vocs or
                 ph_words[i + 2][0] in voicing_dict)
        ):
            if (
                    len(ph_words[i]) >= 4 and
                    ph_words[i][-4:-2] in voi_dict_rev and
                    ph_words[i][-2:] in voi_dict_rev
            ):
                ph_words[i] = ph_words[i][:-4] + \
                              voi_dict_rev[ph_words[i][-4:-2]] + \
                              voi_dict_rev[ph_words[i][-2:]]
    ...
    # put transcriptions of alphabetic substrings and
    # non-alphabetic substrings back together
    res = []
