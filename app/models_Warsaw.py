...
def transcribe_text_Warsaw(text):
    ...
    # phonemic transcription of words taken individually
    # (regional processes not applied)
    ph_words = []
    if words:
        for word in words:
    ...
    for i in range(len(ph_words) - 2):
        if (
                (ph_words[i][-1] in voi_dict_rev or
                 ph_words[i][-1] in ipa_vowels or
                 ph_words[i][-4:] in ipa_vowels) and
                (ph_words[i + 1][0] in voi_dict_rev and
    ...
    # put transcriptions of alphabetic strings
    # and non-alphabetic substrings back together
    res = []
    maxi = max([len(ph_words), len(non_words)])
    if (
            maxi == len(ph_words) and
            len(ph_words) != len(non_words)
    ):
        for i in range(len(ph_words)-1):
